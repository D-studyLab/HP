...
<script setup>
  ...
  const navItems = [
    ...
    { name: '活動実績', path: '/#activities', hash: '#activities' },
  ...
  const setupObserver = () => {
    ...
    if (!container) {
      console.log("Observer setup failed: .content not found");
      return
    }
    ...
    observer = new IntersectionObserver(
      (entries) => {
        console.log("IntersectionObserver entries:", entries); // Log all entries
    ...
    navItems.forEach((item) => {
      ...
      if (element) {
        console.log(`Observing ${item.hash}`);
        observer.observe(element)
      } else {
        console.log(`Could not find element for ${item.hash}`);
      }
    })
  }

  onMounted(() => {
    // nextTick(setupObserver) // 監視の初期化は watch に集約
  ...
  watch(
    () => route.path,
    () => {
      // setTimeout を使って、DOMの描画が完了するのを待つ
